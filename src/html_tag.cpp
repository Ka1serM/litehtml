...
namespace litehtml
{
    ...
    bool litehtml::html_tag::appendChild(const element::ptr& el)
    {
        if(el)
        {
            ...
            el->update_visibility(is_visible());
    ...
    void html_tag::set_attr(const char* _name, const char* _val)
    {
        if(_name && _val)
        {
            ...
            const auto old = m_attrs.find(name);
            if(old != m_attrs.end() && old->second == _val) return;
    ...
    void litehtml::html_tag::compute_styles(bool recursive)
    {
        ...
        // A persistent render tree can reuse clean fragments across document
        // renders, but a computed CSS change invalidates the containing
        // blocks that depend on this element's box.
        if(const auto render = get_render_item()) render->invalidate_layout();
    ...
    void litehtml::html_tag::draw_list_marker(uint_ptr hdc, const position& pos, const std::shared_ptr<render_item>& ri)
    {
        ...
        {
            lm.pos.height = sz_font / 3_px;
            lm.pos.y      = pos.y + ln_height / 2_px - lm.pos.height / 2_px;
            lm.index      = -1;
        }
